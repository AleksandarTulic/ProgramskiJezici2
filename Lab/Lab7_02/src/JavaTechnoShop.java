...
public class JavaTechnoShop{
    public static void main(String []args) throws IOException, ClassNotFoundException {
        ...
        while (true){
            ...
            System.out.println("Prikazati sve racune               [7]");
            System.out.println("Zakljuci kupovinu                  [8]");
            ...
            if ( option == 1 ){
                ...
                File nameFile = new File("D:\\FAKULTET");
                File []listFile = nameFile.listFiles();

                for (File element : listFile){
                    if ( element.isFile() ){
                        System.out.println(element);

                        ObjectInputStream obj = new ObjectInputStream(new FileInputStream(element));
                        racun r = (racun)obj.readObject();
                        r.buy();
                    }
                }
            }
                ...
                arr.buy();
                String dod = LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd.MM.yyyy-HH-mm-ss"));
                dod+=".etf";
                ObjectOutputStream obj = new ObjectOutputStream(new FileOutputStream("D:\\FAKULTET\\" + dod));
                obj.writeObject(arr);
                obj.close();
                break;
            }else{
